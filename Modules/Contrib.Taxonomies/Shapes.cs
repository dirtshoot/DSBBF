...
namespace Contrib.Taxonomies {
    public class Shapes : IShapeTableProvider {
        public void Discover(ShapeTableBuilder builder) {
            builder.Describe("Content")
                .OnDisplaying(displaying => {
                    ...
                    if (term != null) {
                        ...
                        while (curTerm != null) {
                            var alternate = FormatAlternate(term.As<IRoutableAspect>().Path);
            ...
            builder.Describe("Taxonomy_List")
                .OnDisplaying(displaying => {
                    ...
                    metadata.Alternates.Add("Taxonomy_List_" + FormatAlternate(taxonomy.Slug));

                    var curTerm = term;
                    var alternates = new List<string>();
                    while (curTerm != null) {
                        alternates.Add("Taxonomy_List_" + FormatAlternate(term.As<IRoutableAspect>().Path));
                        curTerm = curTerm.Container.As<TermPart>();
                    }
            ...
            builder.Describe("Pager")
                .OnDisplaying(displaying => {
                    ...
                    var alternates = new List<string>();
                    while (curTerm != null) {
                        alternates.Add("Taxonomy_Pager_" + FormatAlternate(term.As<IRoutableAspect>().Path));
                        curTerm = curTerm.Container.As<TermPart>();
                    }
